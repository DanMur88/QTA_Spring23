...
saveRDS(nb_final2, "data/nb_final2")
...
# Save model
saveRDS(svm_train2, "data/svm_train2")
...
# Generate prediction on test set using training set model
pred_svm3 <- predict(svm_train2, newdata = Test2)
...
# Finalise model
svm_final2 <- train(human_labels2 ~ ., 
                    data = ldata2,  
                    method = "svmLinear", 
                    trControl = trainControl(method = "none"),
                    tuneGrid = data.frame(svm_train2$bestTune))
# Save model
saveRDS(svm_final2, "data/svm_final2")

print(svm_final2)

# Run prediction on validation set
svm_pred4 <- predict(svm_final2, newdata = vdata2)
